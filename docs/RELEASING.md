...
현재 배포에 사용하는 Mac에서는 인증서와 공증 프로필이 준비되어 있으므로
GitHub Secrets를 추가로 등록할 필요가 없습니다.

1. 코드 수정과 함께 `docs/releases/0.3.11.md`처럼 새 버전의 설명 파일을 작성합니다.
2. 변경 사항을 `main`에 커밋합니다. 아직 push하지 않아도 됩니다.
3. 프로젝트 폴더에서 다음 명령을 실행합니다.

```bash
./scripts/release.sh 0.3.11
```

버전 갱신 → 테스트 → 앱 빌드 → 서명·Apple 공증 → 버전 커밋과 태그 push →
GitHub 릴리스 공개까지 이어서 실행합니다. 최신 앱은
`dist/local-test/CodexSwitch.app` 한 개로 교체하며 ZIP과 업데이트 피드는 `dist`에 둡니다.
테스트·공증 자체의 시간은 기존과 같고, 명령 사이에 수동으로 이어갈 작업을 없앱니다.

실행에는 `gh auth login`으로 로그인한 GitHub CLI, Xcode, 기존 Developer ID
Application 인증서, Keychain의 `CodexSwitch-notary` 공증 프로필과 `CodexSwitch`
Sparkle 키가 필요합니다. 다른 Mac의 인증서 이름·프로필은 `CODE_SIGN_IDENTITY`와
`NOTARY_PROFILE` 환경 변수로 지정할 수 있습니다.

실패하면 원인을 해결한 뒤 같은 명령으로 재시도합니다. 빌드가 실패하면 자동 변경한
버전 파일을 복원합니다. 버전 커밋 이후 push나 게시가 실패하면 해당 커밋과 초안을
유지하므로 같은 버전으로 이어서 실행할 수 있습니다. 이미 공개한 버전은 덮어쓰지 않습니다.

## GitHub Actions 사용은 선택 사항
...
평소에는 위의 로컬 배포 명령을 사용하면 됩니다. 개인 Mac 없이 GitHub에서
배포하려는 경우에만 아래 Secrets를 설정하고 `Release` 액션을 실행합니다.
...
기준 파일은 **`Resources/Info.plist` 한 개**입니다. 이 파일을 미리 수정하지 않고
`./scripts/release.sh 0.3.11`의 인수로 새 버전을 전달합니다. Actions를 사용할 때는
Run workflow의 **version**에 입력합니다.

| 항목 | 변경 전 예시 | `0.3.11`을 입력한 결과 |
| --- | --- | --- |
| `CFBundleShortVersionString` | `0.3.10` | `0.3.11` — 사용자에게 보이는 버전 |
| `CFBundleVersion` | `14` | `15` — 자동 업데이트가 비교하는 내부 번호 |
| Git tag | `v0.3.10` | `v0.3.11` |

배포 스크립트가 두 값을 갱신한 상태로 빌드·검증한 뒤, 버전 변경 커밋과 태그를
main에 함께 push하고 릴리스를 게시합니다. 같은 버전의 실패 작업을 재시도하면
내부 번호를 중복 증가시키지 않습니다. 이미 공개한 버전은 다시 게시할 수 없습니다.

로컬 배포는 현재 저장소에 버전 커밋이 만들어집니다. Actions에서 배포했다면
다음 로컬 작업 전에 `git pull --ff-only`로 자동 생성된 버전 커밋을 가져오세요.

## 릴리스 설명 작성

**`docs/releases/<버전>.md`**에 GitHub Release에 표시할 내용을 작성하고 코드와
함께 main에 커밋합니다. 로컬 스크립트가 push까지 처리하며, Actions를 쓸 때만
먼저 push합니다. 예를 들어 다음 배포는 `docs/releases/0.3.11.md`입니다.
[기존 0.3.10 설명](releases/0.3.10.md)을 형식 참고용으로 사용할 수 있습니다.

로컬 스크립트와 액션은 이 파일의 Markdown을 그대로 게시합니다. 한국어·영어 설명 모두 한 파일에
