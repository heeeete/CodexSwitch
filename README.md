...
- **사용량 확인** — 일반 Codex의 잔여 비율을 큰 숫자와 막대로 보여줍니다. 기록에 포함된 5시간·주간 창과 재설정까지 남은 시간을 자동 반영합니다.
...
일반 Codex의 로컬 세션 기록을 읽으며, Spark 등 별도 모델의 사용량은 제외합니다. 기록에 주간 창만 있으면 주간만, 5시간 창이 함께 있으면 두 창을 표시합니다. 서버 정책이 바뀌면 Codex가 새 사용량을 기록한 뒤 다음 새로고침에서 반영됩니다. 계정을 전환한 뒤 새 기록이 없으면 사용량 정보 없음으로 표시합니다.

초기화 쿠폰은 현재 계정의 저장된 로그인 토큰을 사용해 OpenAI의 쿠폰 API에서 직접 조회합니다. 계정 로드·전환·새로고침 시 갱신하며, 쿠폰별 남은 기간은 일·시간으로, 하루 미만은 시간·분으로 표시합니다. 만료된 쿠폰은 목록에서 제외합니다.
