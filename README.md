...
- **초기화 쿠폰** — 현재 계정의 사용 가능한 쿠폰과 각 만료일을 만료가 가까운 순서로 보여줍니다. 사용량 조회 설정과 무관하게 자동으로 조회합니다.
- **자동 새로고침** — 메뉴를 닫아도 1분마다 현재 조회 설정으로 사용량을 갱신합니다. 기본값은 켜짐이며 선택은 앱을 다시 실행해도 유지됩니다.
...
다시 시작할까요?”**라고 알려줍니다. **다시 시작**을 누르면 CodexSwitch만 업데이트하고
...
로컬 Codex 세션에서 사용량을 읽습니다. 사용량을 위한 API는 호출하지 않지만, 비활성 계정의 값이 늦게 갱신될 수 있습니다.

초기화 쿠폰은 사용량 조회 모드와 별개로 현재 계정의 저장된 로그인 토큰을 사용해 OpenAI의 쿠폰 API에서 직접 조회합니다. 계정을 불러오거나 전환·새로고침할 때 갱신하며, 사용 가능한 쿠폰의 만료일을 기기 현지 시각으로 표시합니다. 쿠폰이 없을 때와 조회 실패를 구분하고, 쿠폰을 사용하는 기능은 포함하지 않습니다.
