...
struct InstallationView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text(startup.installationError == nil ? L10n.text("CodexSwitch를 설치하고 있어요") : L10n.text("설치를 마치려면 확인이 필요해요"))
                .font(.system(size: 19, weight: .semibold, design: .rounded))
                .padding(.bottom, 10)

            Text(startup.installationError ?? L10n.text("응용 프로그램 폴더에 넣은 뒤 자동으로 열립니다.\n다음 업데이트부터는 앱 안에서 편하게 설치하세요."))
                .font(.system(size: 13))
            ...
            if startup.installationError == nil {
                ProgressView().controlSize(.small)
                    .accessibilityLabel(L10n.text("앱 설치 중"))
            } else {
                HStack(spacing: 12) {
                    Button(L10n.text("종료")) { NSApp.terminate(nil) }
                        .keyboardShortcut(.cancelAction)
                    Button(L10n.text("다시 시도")) { startup.installOrStart() }
